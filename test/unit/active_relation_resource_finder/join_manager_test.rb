...
class JoinTreeTest < ActiveSupport::TestCase
  ...
  def test_no_added_joins
    join_manager = JSONAPI::ActiveRelationResourceFinder::JoinManager.new(resource_klass: PostResource)
  ...
  def test_add_single_join
    filters = {'tags' => ['1']}
    join_manager = JSONAPI::ActiveRelationResourceFinder::JoinManager.new(resource_klass: PostResource, filters: filters)
    records = PostResource.records({})
    records = join_manager.join(records, {})
  ...
  def test_add_single_sort_join
    ...
    join_manager = JSONAPI::ActiveRelationResourceFinder::JoinManager.new(resource_klass: PostResource, sort_criteria: sort_criteria)
    records = PostResource.records({})
    records = join_manager.join(records, {})
  ...
  def test_add_single_sort_and_filter_join
    filters = {'tags' => ['1']}
    sort_criteria = [{field: 'tags.name', direction: :desc}]
    join_manager = JSONAPI::ActiveRelationResourceFinder::JoinManager.new(resource_klass: PostResource, sort_criteria: sort_criteria, filters: filters)
    records = PostResource.records({})
    records = join_manager.join(records, {})
    assert_equal 'SELECT "posts".* FROM "posts" LEFT OUTER JOIN "posts_tags" ON "posts_tags"."post_id" = "posts"."id" LEFT OUTER JOIN "tags" ON "tags"."id" = "posts_tags"."tag_id"', records.to_sql
    assert_hash_equals({alias: 'posts', join_type: :root}, join_manager.source_join_details)
  ...
  def test_add_joins_source_relationship_with_custom_apply
    join_manager = JSONAPI::ActiveRelationResourceFinder::JoinManager.new(resource_klass: Api::V9::PostResource,
  ...
  def test_add_nested_scoped_joins
    ...
    join_manager = JSONAPI::ActiveRelationResourceFinder::JoinManager.new(resource_klass: Api::V9::PostResource, filters: filters)
    records = Api::V9::PostResource.records({})
    records = join_manager.join(records, {})
  ...
  def test_add_joins_with_sub_relationship
    ...
    join_manager = JSONAPI::ActiveRelationResourceFinder::JoinManager.new(resource_klass: Api::V9::PostResource, relationships: relationships,
                                                                          source_relationship: Api::V9::PostResource._relationship(:comments))
    records = Api::V9::PostResource.records({})
    records = join_manager.join(records, {})
  ...
  def test_polymorphic_join_belongs_to_just_source
    join_manager = JSONAPI::ActiveRelationResourceFinder::JoinManager.new(resource_klass: PictureResource,
  ...
  def test_polymorphic_join_belongs_to_filter
    ...
    join_manager = JSONAPI::ActiveRelationResourceFinder::JoinManager.new(resource_klass: PictureResource, filters: filters)
